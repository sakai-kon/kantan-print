...
scaleRange.addEventListener('input', () => {
  if (!pages.length) return;
  const page = pages[currentPage];
  page.scale = Number(scaleRange.value) / 100;
...
grayResetWatcher(grayscaleButton);
function grayResetWatcher(button) {
  button.addEventListener('click', () => {
    if (!pages.length) return;
    commitHistory();
    pages[currentPage].grayscale = !pages[currentPage].grayscale;
    button.textContent = pages[currentPage].grayscale ? '🟦 カラーに戻す' : '⚫ 白黒にする';
    renderCurrentPage();
  });
}
...
textDialog.addEventListener('click', event => {
  if (event.target === textDialog) closeTextDialog();
});
boldTextButton.addEventListener('click', () => {
  dialogBold = !dialogBold;
  boldTextButton.classList.toggle('active', dialogBold);
});
verticalTextButton.addEventListener('click', () => {
  dialogVertical = !dialogVertical;
  verticalTextButton.classList.toggle('active', dialogVertical);
});
...
pageArea.addEventListener('pointerdown', event => {
  if (!cropMode) return;
  if (event.target !== canvas && event.target !== pageArea) return;
...
pageArea.addEventListener('pointermove', event => {
  ...
  const point = canvasPointFromEvent(event);
  cropRect = normalizeRect(cropStart, point);
  updateCropOverlay();
});
...
document.fonts?.ready.then(() => renderCurrentPage());
...
async function loadFile(file) {
  ...
  try {
    ...
    if (isPdf) {
      ...
      pages = [await loadImagePage(file)];
    }
    currentPage = 0;
    history = [];
    future = [];
...
async function loadImagePage(file) {
  ...
  try {
    ...
    normalized.width = img.naturalWidth;
    normalized.height = img.naturalHeight;
...
function renderTexts() {
  ...
  page.texts.forEach(item => {
    ...
    el.style.left = `${item.x * canvas.width}px`;
    el.style.top = `${item.y * canvas.height}px`;
    ...
    el.addEventListener('pointermove', event => {
      ...
      item.x = clamp(drag.x + dx, 0, 1);
      item.y = clamp(drag.y + dy, 0, 1);
      el.style.left = `${item.x * canvas.width}px`;
      el.style.top = `${item.y * canvas.height}px`;
    });
    el.addEventListener('pointerup', () => {
      ...
      if (!isRestoringHistory) {
        history.push(snapshotBeforeDrag || snapshot());
        if (history.length > MAX_HISTORY) history.shift();
        future = [];
      }
      markSelected(item.id);
...
function deleteSelectedText() {
  ...
  const page = pages[currentPage];
  const exists = page.texts.some(text => text.id === selectedTextId);
  if (!exists) return;
  commitHistory();
...
async function openTextDialog(item = null) {
  ...
  dialogBold = item?.bold || false;
  dialogVertical = item?.vertical || false;
  boldTextButton.classList.toggle('active', dialogBold);
  verticalTextButton.classList.toggle('active', dialogVertical);
...
function confirmTextAction() {
  ...
  commitHistory();
  const page = pages[currentPage];
  const fontRatio = Number(textSize.value) / Math.max(1, canvas.width);
  if (editingTextId) {
    const item = page.texts.find(text => text.id === editingTextId);
    if (item) {
      item.value = value;
      item.fontRatio = fontRatio;
      item.color = textColor.value;
      item.bold = dialogBold;
      item.vertical = dialogVertical;
    }
  } else {
    page.texts.push({
...
function applyCrop() {
  if (!pages.length || !cropRect || cropRect.width < 5 || cropRect.height < 5) {
  ...
  const width = Math.min(canvas.width - x, Math.floor(cropRect.width));
  const height = Math.min(canvas.height - y, Math.floor(cropRect.height));
  ...
  const page = pages[currentPage];
  page.baseSrc = out.toDataURL('image/png');
  page.initialSrc = page.initialSrc;
  page.rotation = 0;
  page.scale = 1;
  page.brightness = 100;
  page.contrast = 100;
  ...
  page.texts = page.texts
    .map(text => {
      const px = text.x * canvas.width;
      const py = text.y * canvas.height;
      text.x = (px - x) / width;
      text.y = (py - y) / height;
      text.fontRatio = (text.fontRatio * canvas.width) / width;
      return text;
    })
    .filter(text => text.x >= -0.05 && text.x <= 1.05 && text.y >= -0.05 && text.y <= 1.05)
    .map(text => ({ ...text, x: clamp(text.x, 0, 1), y: clamp(text.y, 0, 1) }));
...
function refreshControls() {
  ...
  grayscaleButton.textContent = page.grayscale ? '🟦 カラーに戻す' : '⚫ 白黒にする';
  updatePageControls();
}
...
async function printAllPages() {
  ...
  printRoot.id = 'printPages';
  printRoot.className = 'print-pages';
  document.body.appendChild(printRoot);
  try {
    for (const page of pages) {
      const image = await loadImage(page.baseSrc);
      const out = document.createElement('canvas');
      ...
      outCtx.rotate(page.rotation * Math.PI / 180);
      outCtx.drawImage(image, -w * page.scale / 2, -h * page.scale / 2, w * page.scale, h * page.scale);
      outCtx.restore();
      outCtx.filter = 'none';

      const sheet = document.createElement('div');
      sheet.className = 'print-sheet';
      sheet.style.width = `${outW}px`;
      sheet.style.height = `${outH}px`;
      sheet.appendChild(out);

      page.texts.forEach(text => {
        const label = document.createElement('div');
        label.className = 'print-text';
        label.textContent = text.value;
        label.style.left = `${text.x * outW}px`;
        label.style.top = `${text.y * outH}px`;
        label.style.fontSize = `${Math.max(10, text.fontRatio * outW)}px`;
        label.style.color = text.color;
        label.style.fontWeight = text.bold ? '800' : '600';
        label.style.writingMode = text.vertical ? 'vertical-rl' : 'horizontal-tb';
